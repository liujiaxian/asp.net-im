    ...
    public class SocketConnection
    {
        ...
        private void Read(IAsyncResult status)
        {
            ...
            try
            {
                ...
                {
                    //var model = JsonConvert.DeserializeObject<t_msg>(messageReceived);
                    //断开连接
                ...
                else
                {
                    ...
                    {
                        var model = JsonConvert.DeserializeObject<t_msg>(messageReceived);
                        logger.Log("接受到的信息 [\"" + model + "\"]");
                        //判断是否登录进来
                        if (model.state == "login")
                        {

                        }
                        else if (model.state=="all")//群发
                        {
                            ...
                            if (tb.Rows.Count>0)
                            {
                                foreach (DataRow row in tb.Rows)
                                {
                                    ...
                                    string sql1 = "insert into t_msg(userName,msg,replyUser,addTime) values(@userName,@msg,@replyUser,@addTime)";
                                    SqlParameter[] pams = { 
                                              new SqlParameter("@userName","admin"),
                                              new SqlParameter("@msg",model.msg),
                                              new SqlParameter("@replyUser",username),
                                              new SqlParameter("@addTime",model.addTime)
                                              };
                                    SqlHelper.ExecuteNonQuery(sql1, pams);
                                }
                            }
                        }
                        else
                        {
                            //发送的信息
                            string sql = "insert into t_msg(userName,msg,replyUser,addTime) values(@userName,@msg,@replyUser,@addTime)";
                            SqlParameter[] pams = { 
                                              new SqlParameter("@userName",model.userName),
                                              new SqlParameter("@msg",model.msg),
                                              new SqlParameter("@replyUser",model.replyUser),
                                              new SqlParameter("@addTime",model.addTime)
                                              };
